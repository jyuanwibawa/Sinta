...
function indonesian_date_only ($timestamp = '', $date_format = 'l, j F Y', $suffix = 'WIB') {
    if (trim ($timestamp) == '')
    {
            $timestamp = time ();
    }
...
function indonesian_date_only_without_days ($timestamp = '', $date_format = 'j F Y', $suffix = 'WIB') {
    if (trim ($timestamp) == '')
    {
            $timestamp = time ();
    }
...
function indonesian_date_tanggal ($timestamp = '', $date_format = 'd F Y') {
    if (trim ($timestamp) == '')
    {
            $timestamp = null;
    }
	else if (trim ($timestamp) == '0000-00-00')
    {
            $timestamp = null;
    }
    elseif (!ctype_digit ($timestamp))
...
function indonesian_date_waktu ($timestamp = '', $date_format = 'H:i:s', $suffix = 'WIB') {
    if (trim ($timestamp) == '')
    {
            $timestamp = time ();
    }
...
function formatTime($time) { // by enrico
    if (!$time) return '';
    ...
    // Menampilkan format yang sesuai
    if ($total_hours > 0 && $total_minutes > 0) {
        return $total_hours . ' Jam ' . $total_minutes . ' Menit';
    } elseif ($total_hours > 0) {
        return $total_hours . ' Jam';
    } else {
